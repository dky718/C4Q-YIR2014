...
window.c4qD3AnimatePie = () ->
  fn = ->
    ...
    t1 = paths.transition().duration(pieTransitionTime)
      .attrTween('d', interpolator)

    t2 = t1.transition().duration(transitionTime)
      .each "start", -> currentSlice = 1
      .each -> d3.selectAll($('.vis-tooltip.one *')).transition().style('opacity', 1)

    t3 = t2.transition().duration(pieTransitionTime).attrTween('d', interpolator)
      .each -> d3.selectAll($('.vis-tooltip.one *')).transition().duration(fadeOutTime).style('opacity', 0.2)

    t4 = t3.transition().duration(transitionTime)
      .each "start", -> currentSlice = 2
      .each -> d3.selectAll($('.vis-tooltip.two *')).transition().style('opacity', 1)

    t5 = t4.transition().duration(pieTransitionTime).attrTween('d', interpolator)
      .each -> d3.selectAll($('.vis-tooltip.two *')).transition().duration(fadeOutTime).style('opacity', 0.2)

    t6 = t5.transition().duration(transitionTime)
    ...
    t9 = t8.transition().duration(pieTransitionTime)
      .each ->
        ...
        highlight.append('path')
          ...
          .attr 'd', arc
        highlight.transition().duration(transitionTime)
          .style('opacity', '1')

    t10 = t9.transition().duration(transitionTime)
